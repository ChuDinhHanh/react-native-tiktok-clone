    ...
    return (
        <TopTab.Navigator
            ...
            <TopTab.Screen
                ...
                {() => (
                    <AllProductComponent type={2} data={NewOfferData} onPressOnProduct={handleOnPressOnProductEvent} />
                )}
            </TopTab.Screen>
                ...
                options={{
                    tabBarLabel: ({ icon, size, focused }: any) => (
                        <RowComponent justify='space-between' alignItems='center'>
                            <View style={{ transform: [{ rotate: '45deg' }] }}><IconIonicons name='ticket' color={'orangered'} size={30} /></View>
                            <TextComponent
                                text='Giảm 50%+'
                                color={focused ? Colors.BLACK : Colors.GREY}
                                fontWeight='bold'
