...
const DotsComponent = React.memo(({ length, currentDot }: { length: number; currentDot: number }) => {
    return (
        <View
                        ...
                        <View
                            style={
                                [
                                    styles.dot,
                                    {
                                        backgroundColor: currentDot === index ? Colors.WHITE : Colors.GREY_2
                                    }
                                ]
                            }
                            key={index}>
...
const CurrentIndex = React.memo(({ length, currentDot }: { length: number; currentDot: number }) => {
    return <View style={{
        position: 'absolute',
...
const BannerComponent = (props: Props) => {
    const { height, widthOfBanner, data, showNode, showIndexNumber, autoScroll } = props;
        ...
        if (isFocusing && autoScroll) {
            counterDown = setInterval(() => {
                if (currentIndexDot !== data.length - 1) {
                    BannerRef.current?.scrollToIndex({ animated: true, index: (currentIndexDot + 1) });
                } else {
    ...
    const renderItem = (item: any) => {
        return (
            <TouchableOpacity
                onPress={() => { console.log("click banner", item.id) }}>
                <View style={{
                    width: widthOfBanner,
                }}>
                    <Image
                        style={[styles.bannerImages, { height }]}
                        source={item.image} />
                </View>
            </TouchableOpacity>
        )
    ...
    return (
        <View>
            ...
            {/* Dot */}
            {
                showNode && <DotsComponent
                    currentDot={currentIndexDot}
                    length={data.length} />
            }
            {/* Current index */}
            {
                showIndexNumber && <CurrentIndex
                    currentDot={currentIndexDot}
                    length={data.length} />
            }
        </View>
